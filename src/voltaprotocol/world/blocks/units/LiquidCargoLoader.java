    ...
    public class LiquidCargoLoaderBuild extends Building{
        ...
        public void updateTile(){
            ...
            if(drone == null && droneId != -1){
                ...
                if(drone != null){
                    if(drone instanceof BuildingTetherc bt) bt.building(this);
                    droneId = -1;
                    restoreTimer = 0f;
                    return;
                }
                restoreTimer += edelta();
                if(restoreTimer < restoreTime) return;
                droneId = -1;
                restoreTimer = 0f;
            }
            ...
            if(drone != null){
                ...
                readiness = Mathf.approachDelta(readiness, 1f, 1f / 60f);
            }
            ...
            if(drone == null){
                ...
                if(buildProgress >= 1f && !Vars.net.client()){
                    if(team.data().countType(unitType) >= Units.getCap(team)) return;
        ...
        public void draw(){
            ...
            if(drone == null){
                Draw.draw(Layer.blockOver, () ->
                    Drawf.construct(this, unitType.fullIcon, 0f,
                        buildProgress, warmup, totalProgress)
                );
            } else {
                Draw.z(Layer.blockOver);

                Color color = defaultPolyColor;
                if(drone.controller() instanceof LiquidCargoAI ai && ai.currentLiquid != null){
                    color = ai.currentLiquid.color;
                } else if(liquids != null && liquids.currentAmount() > 0.001f){
