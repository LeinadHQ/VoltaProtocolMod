...
public class LiquidCargoAI extends AIController {
    ...
    public void updateMovement() {
        ...
        if (liquidAmount <= 0.01f) {
            ...
            if (loaderBuild.liquids != null
                    && unit.within(loaderBuild, transferRange)
                    && loaderBuild.liquids.currentAmount() > 0f) {

                findAnyTarget(loaderBuild);

                if (liquidTarget != null && !Vars.net.client()) {
                    float amountToTake = Math.min(
                        droneLiquidCapacity - liquidAmount,
                        loaderBuild.liquids.get(liquidTarget)
                    );
                    if (amountToTake > 0f) {
            ...
            if (unloadTarget != null) {
                ...
                if (unit.within(unloadTarget, transferRange) && !Vars.net.client()) {
                    float spaceLeft    = unloadTarget.block.liquidCapacity
                                         - unloadTarget.liquids.get(currentLiquid);
    ...
    void findUnloadTarget() {
        unloadTarget = null;
        if (currentLiquid == null) return;

        Seq<LiquidCargoUnloadPointBuild> available = LiquidCargoUnloadPoint.allReceptors.select(
            u -> u.team == unit.team
              && u.isAdded()
              && u.targetLiquid == currentLiquid
              && u.liquids.get(currentLiquid) < u.block.liquidCapacity
        );

        if (available.isEmpty()) return;

        unloadTarget = available.get(targetIndex % available.size);
        targetIndex++;
    }
    ...
    void findAnyTarget(Building loaderBuild) {
        ...
        Seq<LiquidCargoUnloadPointBuild> baseTargets = LiquidCargoUnloadPoint.allReceptors
            .select(u -> u.team == unit.team && u.isAdded());
        ...
        for (Liquid liquid : orderedLiquids) {
            ...
            for (var t : baseTargets) {
                if (t.targetLiquid == liquid && t.liquids.get(liquid) < t.block.liquidCapacity) {
                    targets.add(t);
                }
            }
            if (targets.size > 0) {
    ...
    boolean isValidTarget(LiquidCargoUnloadPointBuild target) {
        return target != null
            && target.isAdded()
            && currentLiquid != null
            && target.targetLiquid == currentLiquid
